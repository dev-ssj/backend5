    ...
    public String refreshToken(String refreshToken){
        ...
        if(jwtTokenProvider.validateToken(refreshToken)) {
            ...
            Auth auth = authRepository.findByRefreshToken(refreshToken).orElseThrow(
                    () -> new IllegalArgumentException("해당 RefreshToken을 찾을 수 없습니다. Refresh_token = " + refreshToken));

            //있으면 인증객체를 만들어서 새로운 토큰 발급
            String newAccessToken = jwtTokenProvider.generatedToken(
